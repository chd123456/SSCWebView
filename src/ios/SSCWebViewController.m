...
@interface SSCWebViewController ()
...
- (UIBarButtonItem *)backItem
{
    if (!_backItem) {
        ...
        UIImage *image = [UIImage imageNamed:@"back_sscwebview"];
...
{
    if (!_closeItem) {
        _closeItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"close_sscwebview"] style:UIBarButtonItemStylePlain target:self action:@selector(closeNative)];
        _closeItem.tintColor = [UIColor colorWithRed:242.0/255.0 green:82.0/255.0 blue:87/255.0 alpha:1.0];
    }
    return _closeItem;
}
...
- (void)addProgressBar
{
    ...
    self.progressView.progressTintColor = [UIColor colorWithRed:242.0/255.0 green:82.0/255.0 blue:87/255.0 alpha:1.0]; //进度色
